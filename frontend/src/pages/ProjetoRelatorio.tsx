        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="btn-secondary" onClick={() => navigate(`/projetos/${id}/indicadores`)}>Voltar aos Indicadores</button>
              <button className="btn btn-outline" onClick={() => {
                const filename = `relatorio_indicadores_${data.nome_projeto.replace(/\s+/g, '_')}.pdf`;
                downloadPDF(`${import.meta.env.VITE_API_URL || '/api'}/pdf/projetos/${id}/indicadores`, filename);
              }}>Exportar PDF</button>
              <button className="btn btn-secondary" onClick={() => navigate(`/projetos/${id}/relatorio-tela-cheia`)}>Abrir em Tela Cheia</button>
            </div>
          </div>
        </div>
      </div>
